...
#p <- groupClade(p, clades, group_name='subtree') + aes(color=subtree)+geom_tiplab(aes(label=label),color='#000000',size = 4)+ scale_color_manual(values = c(cols)) 
p <- groupClade(p, clades, group_name='subtree')+geom_tiplab(aes(label=label),color='#000000',size = 4)
...
get_pie <-function (){
    ...
    for (i in node){
            ...
            new=length(subset(sampledata ,sampledata$Node==i)$Chr)
            new_0=length(subset(sampledata ,sampledata$Node==i & sampledata$Var_type == 0 )$Node)
            new_1=length(subset(sampledata ,sampledata$Node==i  & sampledata$Var_Type == 1)$Node)
            no=num-new
            if (new == 0){
		print (new)
	    }else{	
